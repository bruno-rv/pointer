...
- C owns `FirstUseGuidePresenting.swift`, the `FirstUseGuidePresenting`
  ...
  exclusively F-owned. C also predeclares the canonical public
  `FirstUseGuideStateStoring` seam in
  `Sources/PointerAppKit/Help/FirstUseGuideStateStoring.swift`; C consumes it
  through the controller and D consumes the same declaration.
...
- Workstream D implements the concrete first-use guide in
  ...
  D consumes the predeclared `FirstUseGuideStateStoring` protocol without
  redeclaring or extending it, and owns
...
- `PerformanceComparisonHarness.swift` consumes two validated
  ...
  metric has its canonical `PerformanceMetricUnit`; `budgetLimit` is optional,
  finite and positive only for `combinedFrame` (16.7 ms), `responsiveness`
  (100 ms), and `inputToVisible` (100 ms), and nil for other metrics. Samples
  are finite and strictly positive for both baseline and candidate, with
