...
- CommandRouter exposes read-only `pendingShortcutDisplayName` and `pendingShortcutGuidance` views over `HotKeyController.pendingPreset?.displayName`; the latter is the shared concise `Press <display name> within 5 seconds to confirm` message used by the palette and menu while the old active shortcut remains selected.
...
- `MenuBarController.refresh` keeps the menu usable when the palette is hidden or no display is connected: annotation mode is disabled with the exact unavailable help/value, while the status item uses a native warning image and `No presentation display connected` tooltip/accessibility value. A pending shortcut marks its candidate menu item `Pending`/mixed and exposes the same router guidance on the parent and status item; successful delivery replaces the selected active item, and timeout/registration errors remain actionable.
...
- Pending shortcut status is represented by the canonical `ShortcutPreset.displayName` through `CommandRouter.pendingShortcutDisplayName`/`pendingShortcutGuidance`; no alternate raw-ID display name or persistent status chrome is introduced. The menu fallback remains actionable with a native no-display warning even when the palette is hidden.
