...
reload fails. The non-modal `.nonactivatingPanel` remains floating but can
become key without becoming main; after ordering front it makes Done the first
responder and keeps Return as its key equivalent. Escape remains
controller-owned and dismiss-first. The semantic guide background is reapplied
under the changed effective appearance.
